...
-- porque essa loja encontra-se em uma região onde a tributação foi modificada recente.
...
-- 2. O setor de controle de produtos quer fazer uma análise para descobrir quais são os produtos que possuem um UnitPrice maior que o UnitPrice do produto
-- de ID igual a 1893.
...
-- 3. A empresa Contoso criou um programa de bonificação chamado "Todos por 1". Este programa consistia no seguinte: 1 funcionário 
-- seria escolhido ao final do ano como o funcionário destaque, só que a bonificação seria recebida por todos da área daquele funcionário 
-- em particular. O objetivo desse programa seria o de incentivar a colaboração coletiva entre os funcionários de uma mesma área. 
...
-- seriam beneficiados com o programa.
 ...
 -- O seu objetivo é realizar uma consulta à tabela DimEmployee e retornar todos os funcionários da área "vencedora" para que o setor Financeiro 
...
-- 4. Faça uma query que retorne os clientes que recebem um salario anual acima da média. A sua query deve retornar as colunas
...
-- 5. A ação de desconto da Asian Holiday Promotion foi uma das mais bem sucedidas da empresa. Agora, a Contoso quer entender um pouco melhor
-- sobre o perfil dos clientes que compraram produtos nessa promoção.
...
-- 6. A empresa emplementou um programa de fidelização de clientes empresariais. Todos aqueles que comprarem mais de 3000 unidades de 
...
-- 9. Crie uma CTE que seja o agrupamento da tabela DimProduct, armazenando o total de produtos por marca. Em seguida, faça um SELECT nessa CTE 
-- descobrindo qual é a quantidade máxima de produtos para uma marca. Chame esta CTE de CTE_QntProdutosPorMarca.
